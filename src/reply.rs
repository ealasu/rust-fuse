...
mod test {
    ...
    fn reply_entry () {
        ...
        let attr = FileAttr { ino: 0x11, size: 0x22, blocks: 0x33, atime: time, mtime: time, ctime: time, crtime: time,
            kind: FileType::TypeFile, perm: USER_FILE, nlink: 0x55, uid: 0x66, gid: 0x77, rdev: 0x88, flags: 0x99 };
    ...
    fn reply_directory () {
        ...
        reply.add(0xaabb, 1, FileType::TypeDirectory, &PosixPath::new("hello"));
        reply.add(0xccdd, 2, FileType::TypeFile, &PosixPath::new("world.rs"));
        reply.ok();
    }
}
